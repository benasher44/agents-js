...
describe('AMD', () => {
  ...
  // Ref: python tests/test_amd_classifier.py (PR #5637) — late STT transcripts
  // arriving inside the short-speech window must cancel the pre-baked HUMAN /
  // short_greeting timer and let the LLM verify with the long_speech budget.
  it('should not fire short_greeting when a transcript arrives late', async () => {
    ...
    llm.on('error', () => {});
    const amd = new AMD(asAgentSession(session), { llm, detectionTimeoutMs: 5_000 });
    ...
    // Transcript arrives 200ms after speech end, well inside the 500ms HUMAN
